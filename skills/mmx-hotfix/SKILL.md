...
description: >
  ...
  Disparador: Cuando el usuario ejecuta /mmx-hotfix para un cambio directo.
license: MIT
metadata:
...
1. Crea el directorio si es un ticket nuevo: `mkdir -p .memex/changes/{nombre-del-cambio}`

2. Usa `write_file` para crear un archivo base `.memex/changes/{nombre-del-cambio}/state.ini` con este contenido exacto:
...
`python3 scripts/mmx_state_manager.py begin --change {nombre-del-cambio} --phase apply`
...
`python3 scripts/mmx_state_manager.py commit --change {nombre-del-cambio} --next-phase verify`
