...
description: >
  ...
  Disparador: Cuando el usuario ejecuta /mmx-archive para archivar un cambio después de la implementación y verificación.
license: MIT
metadata:
...
Skill responsable del **ARCHIVADO**. Fusiona las specs delta en las specs principales (fuente de verdad), y luego mueve la carpeta del cambio al archivo. Completa el ciclo del agente.
...
Para cada spec delta en `.memex/changes/{nombre-del-cambio}/specs/`:

#### Si Existe la Spec Principal (`.memex/specs/{dominio}/spec.md`)
...
La spec delta es una spec completa. Copiarla directamente a `.memex/specs/{dominio}/spec.md`.

### Paso 3: Mover al Archivo

```text
.memex/changes/{nombre-del-cambio}/
  → .memex/changes/archive/YYYY-MM-DD-{nombre-del-cambio}/
```
...
**Archivado en**: .memex/changes/archive/{YYYY-MM-DD}-{nombre-del-cambio}/
...
- Si `.memex/changes/archive/` no existe, crearlo
- Aplicar cualquier `rules.archive` de `.memex/config.yaml`
