...
description: >
  ...
  Disparador: Cuando el usuario ejecuta /mmx-apply para implementar tareas.
license: MIT
metadata:
...
├── .memex/config.yaml → rules.apply.tdd (true/false — máxima prioridad)
...
- Aplicar cualquier `rules.apply` de `.memex/config.yaml`
