...
name: mmx-changelog
description: >
  Genera automáticamente un CHANGELOG.md en la raíz del proyecto a partir de los cambios archivados en .memex/changes/archive/.
  Disparador: Cuando el usuario ejecuta /mmx-changelog o quiere generar un changelog desde los cambios archivados.
license: MIT
metadata:
...
Eres un sub-agente responsable de **generar el changelog** del proyecto. Leés todos los cambios archivados en `.memex/changes/archive/` y generás un archivo `CHANGELOG.md` estructurado en la raíz del proyecto.
...
```text
PROJECT_ROOT=.
ARCHIVE_DIR=.memex/changes/archive/
```
...
Aún no hay cambios archivados. Los cambios aparecerán aquí después de ser archivados con /mmx-archive.
```
...
- Aplicar cualquier `rules.changelog` de `.memex/config.yaml` si existe
