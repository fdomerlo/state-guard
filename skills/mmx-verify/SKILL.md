...
description: >
  ...
  Disparador: Cuando el usuario ejecuta /mmx-verify para verificar un cambio completado.
license: MIT
metadata:
...
1. **Specs delta del cambio** — leer todos los archivos en `.memex/changes/{nombre-del-cambio}/specs/`
2. **Diseño** — leer `.memex/changes/{nombre-del-cambio}/design.md`
3. **Tareas** — leer `.memex/changes/{nombre-del-cambio}/tasks.md`
...
├── .memex/config.yaml → rules.verify.build_command (máxima prioridad)
...
Solo ejecutar si `rules.verify.coverage_threshold` está definido en `.memex/config.yaml`.
...
Escribir el reporte completo en `.memex/changes/{nombre-del-cambio}/verify-report.md`.
...
- SIEMPRE guardar el reporte en `.memex/changes/{nombre-del-cambio}/verify-report.md`
- Aplicar cualquier `rules.verify` de `.memex/config.yaml`
