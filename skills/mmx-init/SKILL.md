...
description: >
  ...
  Disparador: Cuando el usuario quiere inicializar Memex en un proyecto, o dice "iniciar memex", "memex init".
license: MIT
metadata:
...
Consultar `skills/_shared/mmx-convention.md` para el schema de `state.ini`.
...
Basándote en lo detectado, crea el archivo `.memex/config.yaml`:
...
Como paso final, ejecuta `skills/mmx-skill-registry/scan.sh` (o inicializa el archivo `.memex/skill-registry.md` vacío) para habilitar el descubrimiento de skills.
...
### Estructura Creada
- .memex/config.yaml ← Configuración del proyecto con contexto detectado
- .memex/specs/      ← Listo para especificaciones
- .memex/changes/    ← Listo para propuestas de cambio

### Próximos Pasos
Listo para /mmx-explore {tema} o /mmx-new {nombre-del-cambio}.
```

## Reglas

- NUNCA crear archivos de spec de relleno — las specs se crean mediante mmx-spec durante un cambio
- SIEMPRE detectar el stack tecnológico real, nunca asumir
- Si el proyecto ya tiene un directorio `.memex/`, reportar qué existe y preguntar al orquestador si debe actualizarse
