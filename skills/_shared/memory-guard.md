...
4. Persistí el artefacto en disco Y actualizá el estado invocando `mmx_state_manager.py` en la terminal
...
3. Vos actualizás el estado invocando `mmx_state_manager.py` en la terminal (el sub-agente NO toca state.ini)
...
- Leer estado (vía `mmx_state_manager.py status`) y actualizar invocando `mmx_state_manager.py` en la terminal
- Ejecutar fases inline (cargando SKILL.md)

### Lo que delegás (solo si el host lo soporta)

- Fases pesadas de `mmx-apply` (> 10 tareas)
...
1. **Auto-Checkpoint Determinístico:** `cmd_commit` genera y persiste un `session_summary` mínimo con el estado real del DAG (fase completada, siguiente, completadas, pendientes). Esto garantiza warm-boot sin depender de que vos ejecutes `/mmx-checkpoint`.
...
```text
Invocar: mmx_state_manager.py status --change {nombre-del-cambio}
```
...
1. Leé `.memex/changes/{change-name}/state.ini` (vía `status`, ya lo hiciste en el Paso 0).
...
- `mmx-convention.md` — carpetas y rutas exactas.
- `mmx-skill-registry` — escanea skills personalizadas (globales y locales).
