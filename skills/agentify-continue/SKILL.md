---
name: agentify-continue
description: >
  Continúa un cambio transaccional desde donde se quedó
  Disparador: Cuando el usuario ejecuta /agentify-continue.
license: MIT
metadata:
...
|-------------|-------------------|
| `explore`   | agentify-explore       |
| `propose`   | agentify-propose        |
| `spec`      | agentify-spec           |
| `design`    | agentify-design         |
| `tasks`     | agentify-tasks          |
| `apply`     | agentify-apply          |
| `verify`    | agentify-verify         |
| `archive`   | agentify-archive        |

La transacción (BEGIN/COMMIT/ROLLBACK) la maneja la propia skill invocada, según `transaction-protocol.md`. `agentify-continue` no toma el lock de fase directamente — delega esa responsabilidad a la skill de la fase correspondiente.
