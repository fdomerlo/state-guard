...
# ============================================================================
# Memex: Memory Guard — Install Script Tests
...
test_claude_code_skill_count() {
    ...
    assert_eq "20" "$count" "Expected exactly 20 skills for Claude Code"
}
...
test_opencode_commands_path_substituted() {
    bash "$INSTALL_SCRIPT" --agent opencode > /dev/null 2>&1
    local cmd_file="$HOME/.config/opencode/commands/mmx-apply.md"
    ...
    if grep -q '{{SKILLS_PATH}}' "$cmd_file"; then
        echo "Placeholder {{SKILLS_PATH}} was NOT substituted in mmx-apply.md"
        return 1
    fi
    # Must contain the actual skills path
    grep -q '.config/opencode/skills' "$cmd_file" || {
        echo "Expected .config/opencode/skills path in mmx-apply.md"
        return 1
    }
}
...
test_project_local_skill_count() {
    ...
    assert_eq "20" "$count" "Expected exactly 20 skills for project-local"
}
...
test_custom_path_skill_count() {
    ...
    assert_eq "20" "$count" "Expected exactly 20 skills for custom path"
}
...
test_all_global_total_skill_count() {
    ...
    assert_eq "60" "$total" "Expected 63 total SKILL.md files across all targets"
}
...
test_output_shows_install_count() {
    local output
    output=$(bash "$INSTALL_SCRIPT" --agent claude-code 2>&1)
    echo "$output" | grep -q "20 skills installed" || {
        echo "Output missing '20 skills installed' message"
        return 1
    }
}
...
test_overwrite_stale_skill() {
    ...
    mkdir -p "$HOME/.claude/skills/mmx-apply"
    echo "stale" > "$HOME/.claude/skills/mmx-apply/SKILL.md"
    bash "$INSTALL_SCRIPT" --agent claude-code > /dev/null 2>&1
    # Should be replaced with actual content (not "stale")
    local content
    content=$(head -c 5 "$HOME/.claude/skills/mmx-apply/SKILL.md")
    if [[ "$content" == "stale" ]]; then
        echo "SKILL.md was NOT overwritten — still contains stale data"
        return 1
    fi
    assert_file_not_empty "$HOME/.claude/skills/mmx-apply/SKILL.md"
}
...
echo ""
echo -e "${CYAN}${BOLD}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}${BOLD}║    Memex: Memory Guard — Install Script Tests           ║${NC}"
echo -e "${CYAN}${BOLD}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
run_test "Installs all 20 skills to ~/.claude/skills" test_install_claude_code
run_test "Exactly 20 SKILL.md files" test_claude_code_skill_count
echo ""
...
run_test "Installs all 20 skills to ./skills/" test_install_project_local
run_test "Exactly 20 SKILL.md files" test_project_local_skill_count
echo ""

echo -e "${BOLD}Custom path${NC}"
run_test "Installs to arbitrary custom path" test_custom_path
run_test "Exactly 20 SKILL.md files" test_custom_path_skill_count
