...
Para cada spec delta en `.state-guard/changes/{nombre-del-cambio}/specs/`:
...
La spec delta es una spec completa. Copiarla directamente a `.state-guard/specs/{dominio}/spec.md`.

### Paso 3: Mover al Archivo

```text
.state-guard/changes/{nombre-del-cambio}/
  → .state-guard/changes/archive/YYYY-MM-DD-{nombre-del-cambio}/
```
...
**Archivado en**: .state-guard/changes/archive/{YYYY-MM-DD}-{nombre-del-cambio}/
...
- Si `.state-guard/changes/archive/` no existe, crearlo
- Aplicar cualquier `rules.archive` de `.state-guard/config.yaml`
