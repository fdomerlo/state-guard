# Manual Técnico — Memex

Este manual cubre la arquitectura técnica, configuración y flujos avanzados del sistema Memex.
...
Memory Guard (memory-guard.md)
    ...
    │
    ├── Ejecuta inline → mmx-explore   (carga SKILL.md como instrucciones)
    ├── Ejecuta inline → mmx-propose
    ├── Ejecuta inline → mmx-spec
    ├── Ejecuta inline → mmx-design
    ├── Ejecuta inline → mmx-tasks
    ├── Ejecuta inline → mmx-apply     (delega si > 10 tareas y host soporta)
    ├── Ejecuta inline → mmx-verify
    └── Ejecuta inline → mmx-archive
```
...
| `mmx-phase-common.md` | Protocolo de transacción común a todas las skills de fase |
...
1. La fase es `mmx-apply` con más de 10 tareas pendientes, **Y**
...
- Script bash POSIX en `skills/mmx-skill-registry/scan.sh`
- Índice generado en `.memex/skill-registry.md`
...
Además, el empaquetador reescribe dinámicamente las directivas de los *slash commands* (como `mmx-apply.md`) para usar lenguaje imperativo (ej. `INSTRUCCIÓN CRÍTICA: DEBES usar tu herramienta read_file INMEDIATAMENTE en la ruta...`), forzando al modelo a realizar el *tool-calling* esperado.
...
```text
.memex/changes/{nombre-del-cambio}/state.ini
```

**Schema (formato INI, manejado por `mmx_state_manager.py`):**
...
[Session]
session_summary = ...      ; opcional — bloque generado por mmx-checkpoint, ≤500 tokens (enforced en código: máx 2000 chars)
```
...
### /mmx-split — División de Proposals

El comando `/mmx-split` analiza una proposal monolítica y la divide en sub-cambios manejables.
...
```text
/mmx-split mi-cambio-grande
```

### /mmx-ff — Avance Rápido (Fast-Forward)

El comando `/mmx-ff` permite ejecutar secuencialmente las fases de planificación (`propose`, `spec`, `design`, `tasks`).
...
A diferencia de pedirle al LLM que "haga todas las fases de una vez" en un solo prompt (lo que corrompe el DAG), `/mmx-ff` ejecuta 4 transacciones secuenciales independientes. Cada fase tiene su propio ciclo BEGIN → COMMIT, y si el agente crashea entre la transacción 2 y la 3, el Recovery Protocol continúa automáticamente desde donde quedó.

### /mmx-checkpoint — Guardado de Estado

El comando `/mmx-checkpoint` genera un **bloque YAML estructurado** analizando proactivamente
...
- Al interrumpir un lote de `mmx-apply` para preservar el estado
...
### /mmx-archive — Cierre de Cambios

El comando `/mmx-archive` cierra un cambio: fusiona las specs delta en las specs principales y mueve el cambio al archivo.

**Flujo Obligatorio:**

1. Ejecutar `/mmx-verify` y asegurar que todo es correcto.
2. Realizar un `git commit` de todos los cambios de código y especificaciones.
3. Ejecutar `/mmx-archive`.
...
### /mmx-review — Auditoría Estática

El comando `/mmx-review` compara el código implementado contra las especificaciones sin ejecutar tests.
...
```text
/mmx-review mi-cambio
```

### /mmx-rollback — Revertir un Cambio

El comando `/mmx-rollback` purga la carpeta del cambio y restaura los archivos modificados desde git.
...
Memex soporta múltiples agentes de IA. El Memory Guard se adapta automáticamente a las capacidades de cada host:
...
El framework de Memex es extensible mediante "Custom Skills", permitiendo integrar herramientas especializadas que no son parte nativa del framework (por ejemplo, herramientas de desarrollo frontend, diseño o base de datos).
...
Esto escaneará las rutas global y local, y actualizará el archivo de repositorio local en `.memex/skill-registry.md`.

### 4. Uso por el Memory Guard

El Memory Guard lee `.memex/skill-registry.md` al inicializar contexto y mapea cada entrada como una herramienta ejecutable. Al analizar la necesidad de un usuario, se basará en atributos declarados como `name` y `description` para cargar proactivamente la skill relevante.
...
1. Verificar que `state.ini` existe en `.memex/changes/{change-name}/`
2. Verificar `txn_status` vía `mmx_state_manager.py status`: si es `in_progress`, hay una transacción incompleta.
3. Ejecutar `/mmx-continue` para que el Recovery Protocol intente resolver automáticamente.

### Los artefactos no persisten

1. Verificar que el directorio `.memex/` existe
2. Revisar permisos de escritura
3. Confirmar que el cambio tiene un `state.ini` válido

### Transacción incompleta detectada

1. El Recovery Protocol intenta resolver automáticamente al ejecutar `/mmx-continue`
2. Como último recurso, editar manualmente `state.ini`: setear `txn_status = idle`, `txn_phase = None`

### Conflictos entre cambios

1. Usar `/mmx-status` para ver todos los cambios activos (incluye columna de estado transaccional)
2. Archivar cambios completados antes de iniciar nuevos
3. No trabajar en el mismo cambio desde múltiples sesiones

---

*Manual técnico — Memex v2.0 — Arquitectura Memory Guard*
