...
- **Memory Guard**: Nuevo contrato unificado (`_shared/memory-guard.md`) que reemplaza a `mmx-core.md`, `mmx-delegation.md` y `mmx-state.md`. El agente ahora ejecuta fases inline con delegación inteligente en lugar de despachar todo a sub-agentes CLI.
...
- **state.yaml v2**: Schema extendido con campos transaccionales (`schema_version`, `txn_status`, `txn_phase`, `txn_started_at`). Migración automática v1→v2 vía `mmx-fix`.
...
- **Context Injection**: Simplificado `mmx-context.md` → `context-injection.md` eliminando la distinción orquestador/sub-agente.
...
- **Meta-skills** (mmx-new, mmx-continue, mmx-ff): Refactorizadas para ejecución inline directa con transacciones secuenciales.
- **mmx-checkpoint**: Ahora opera en modo dual (automático post-COMMIT + manual bajo demanda).
- **mmx-fix**: Añadida migración v1→v2 y resolución de transacciones incompletas.
- **mmx-status**: Muestra `txn_status` en la tabla de estado.
...
- **Estructura del agente**: Inicialización de la estructura del agente en el proyecto (`/mmx-init`) creando `openspec/config.yaml` y generando el índice de habilidades `.memex/skill-registry.md`.


### Removed

- `_shared/mmx-core.md` — Absorbido en `memory-guard.md`
- `_shared/mmx-delegation.md` — Absorbido en `memory-guard.md`
- `_shared/mmx-state.md` — Absorbido en `transaction-protocol.md`
- `_shared/execution-contract.md` — Absorbido en `transaction-protocol.md`
- `_shared/mmx-commands.md` — Absorbido en `memory-guard.md`
- `_shared/mmx-context.md` — Reemplazado por `context-injection.md`
